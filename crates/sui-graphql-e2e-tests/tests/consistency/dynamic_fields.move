...
{
  parent_version_2_no_dof_but_will_show_dof: object(address: "@{obj_2_1}", version: 2) {
    address
    dynamicFields {
      ...DynamicFieldsSelect
    }
  ...
  parent_version_3_has_dof: object(address: "@{obj_2_1}", version: 3) {
    ...
    child_dof_state_consistent: dynamicObjectField(name: {type: "u64", bcs: "pAEAAAAAAAA="}) {
      value {
        ... on MoveObject {
          address
          # Note that the following will be incorrect until fixed - we need to look up the object's
          # contents using its df_object_id, but its parent is technically the wrapper's parent
          owner {
            ... on Parent {
              ...ParentSelect
            }
