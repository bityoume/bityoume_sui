...
impl Object {
    ...
    pub(crate) async fn paginate_subtype<T: OutputType>(
        ...
        let response = db
            .execute_repeatable(move |conn| {
                ...
                let result = page.paginate_raw_query::<StoredHistoryObject, _>(
                    ...
                    move |element| {
                        element.map(|obj| {
                            Cursor::new(HistoricalObjectCursor::new(obj.object_id.clone(), rhs))
                        })
                    },
                    move || {
        ...
        for stored in results {
            let cursor = Cursor::new(HistoricalObjectCursor::new(stored.object_id.clone(), rhs))
                .encode_cursor();
            let object = Object::try_from_stored_history_object(stored, Some(rhs))?;
